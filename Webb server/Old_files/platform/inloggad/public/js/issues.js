...
parent.$.fancybox.close();
	...
	var newName = document.getElementById("newname-textbox").value;            
	var newUserid = document.getElementById("newuserid-textbox").value;
	var newCategoriesText = document.getElementById("newcategories-textbox").value;
//	var newCategories = newCategoriesText.splice(" ");                                        //ska användas sen
	
	var newEmail = document.getElementById("newemail-textbox").value;
	var newArea = document.getElementById("newarea-textbox").value;
    $.post("http://stsitkand.student.it.uu.se/platform/inloggad/public/database/UpdateMongoDB.php", {
         
        action: "createCompany",
        name: newName,
		userId: newUserid,
		categories: newCategoriesText,
		email: newEmail,
		area: newArea 
    }, function(data) {
		var dataObj = JSON.parse(data);
...
function removeIssue2(index){
	...
	dataSet = [];
	var columnTextLenght = 20;                                                                     //här väljer man hur många tecken som ska få synas av kommentaren och beskrivningen
		...
		for (var j = 0; j < categories.length; j++) {
			...
			//	console.log(markers[i].status);
				...
				if (markers[i].description.length > columnTextLenght){
					...
					shortDesc = (markers[i].description);
				}
				
				var shortComment = "";
				if (markers[i].statusComment.length > columnTextLenght){
					for(var k = 0; k < columnTextLenght-1; k++){
						shortComment += (""+markers[i].statusComment[k]);
					}
					//shortComment += " ...";
				}else {
					shortComment = (markers[i].statusComment);
				}
