...
//--------------------------SKAPAR MARKERSARRAY FRÅN DATABASEN FÖR DATA ÖVER NODER----//
function createMarkerArrayNodesData(markerData, markersTemp, category){

	var currentMarker = [];
	
	currentMarker.category = category;
	currentMarker.Data = markerData;	
...
function circleToMap(){
    ...
    markers[i].postion = new google.maps.LatLng(markers[i].latitude, markers[i].longitude);
    if(i==0){ var amount = 10; var color= '#00ccff'}
    if(i==2){ var amount = 5; var color= '#005eff'}
    if(i==1){ var amount = 3; var color= '#0000FF'}
    var circleOptions = {
      strokeColor: color,
      strokeOpacity: 0.8,
      strokeWeight: 2,
