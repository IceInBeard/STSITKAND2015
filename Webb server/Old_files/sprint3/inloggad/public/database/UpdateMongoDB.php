...
if($action == "removePicture"){	
  $collection1->update(array("_id"=> new Mongoid($id)), array('$set'=>array("Picture"=>$empty)));	
}
if($action == "changePriority"){ 
  $collection1->update(array("_id"=> new Mongoid($id)), array('$set'=>array("Priority"=>$priority))); 
}
