...
function tweetDataparser($rawdata,$criteriafunction1,$param1,$param2){
    ...
    foreach($rawdata as $docs){
        ...
        'user' => $docs['user_name'], 'date' => datemakerStandard($docs['created_at']) ,
		'jsdate' => datemakerSend($docs['created_at']),'value' => 1, 'id' => $docs['_id']
        );
...
$datecheck= function($date,$bounds){
    return (new DateTime(datemakerStandard($date)) >= new DateTime($bounds[0]) && new DateTime(datemakerStandard($date)) < new DateTime($bounds[1] . "tomorrow"));
};
