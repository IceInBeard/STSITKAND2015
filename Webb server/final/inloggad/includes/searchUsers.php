...
include_once 'psl-config.php';
    ...
    if ($result->num_rows > 0) {
        ...
        <th class='user-meny'>Byt lösenord</th>
        ...
        while($row = $result->fetch_assoc()) {
            ...
            <td class='user-meny'><button id='removeButton' class='fancybox fancybox.inline' href='#changePass".$row['id']."'> Byt lösenord </button></td>
            ...
            <script>
                function confirmButton".$row['id']."() {
                    var x;
                    if (confirm('Vill du ta bort användaren ".$row['username']."?') == true) {
    ...
    <div class='login-head'>
        ...
        <form   class='loginForm' action='../includes/editUser.inc.php?id=.".$row['id']."' 
                method='post' 
                name='edit_form'>
            ...
            the fields are entered in a correct way. -->
                <button type='button' class='button' onclick='return regformhashEditUser(this.form,
                                   this.form.username,
                                   this.form.email,
                                   this.form.admin);'>Ändra'</button>
    ...
    $changePassForm.="<div style='display:none;' id='changePass".$row['id']."' >
                
                
   
    <div class='login-head'>
        <h1>Byt lösenord användare</h1>
         <div class='alert-close'> </div>           
    </div>

        <!-- Creates the form for editing users. When the submit button is pressed, it uses ../includes/adminChangePass.inc.php to make
        the changes in the database -->
        <form   class='loginForm' action='../includes/adminChangePass.inc.php' 
                method='post' 
                name='edit_form'>
                
                
                <p type='text' name='id' class='text' value='".$row['id']."'>Du redigerar användaren: ".$row['username']." </p>
            ...
            are entered in the box -->
            <li>
                <input style='color:#000;' type='password' name='newPass' class='text' value=''>
            </li>

            <li>
                <input style='color:#000;' type='password' name='confirmpass' class='text' value=''>
            </li>
                <div class='clear'> </div>
            ...
            <div class='submit'>
               ...
               <!-- The submit button. When pressed, calls the function regformhashEditUser in js/forms.js to check that 
            the fields are entered in a correct way. -->
                <button type='button' class='button' onclick='return regformhashAdminChangePass(this.form,
                                   this.form.newPass,
                                   this.form.confirmpass);'> Ändra </button>
